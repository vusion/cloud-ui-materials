/// <reference types="@nasl/types" />
namespace extensions.cw_dynamic_form_library.viewComponents {
  const { Component, Prop, ViewComponent, Slot, Method, Param, Event, ViewComponentOptions } = nasl.ui;
  type ViewComponent = nasl.ui.ViewComponent;

  @ExtensionComponent({
    type: 'pc',
    ideusage: {
      idetype: 'element',
    }
  })
  @Component({
    title: '开关',
    description: '开关',
  })
  export class CwAntdSwitch extends ViewComponent {
    constructor(options?: Partial<CwAntdSwitchOptions>) {
      super();
    }
  }

  export class CwAntdSwitchOptions extends ViewComponentOptions {
    @Prop({
      title: '值',
      description: '需要传入的值',
      setter: {
        concept: 'SwitchSetter'
      }
    })
    value: nasl.core.Boolean = false;

    @Event({
      title: '值改变事件',
      description: '值改变事件'
    })
    onChange: (value: nasl.core.Boolean) => void;
  }
}
