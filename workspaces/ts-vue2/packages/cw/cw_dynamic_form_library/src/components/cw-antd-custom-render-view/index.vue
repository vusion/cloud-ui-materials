<template>
  <div>
    <div v-if="inIDE" style="width: 100%; height: 300px; display: flex; justify-content: center; align-items: center">动态绚烂组件</div>
    <a-form
      :colon="false"
      :style="customStyle"
    ...
    >
      <div style="display: flex; flex-wrap: wrap">
        <a-col
          class="cell-room"
          :class="[item.belong ? item.belong + '-type-room' : '']"
          :sm="23"
          :lg="{ span: item.span * cellSpan }"
          v-if="evalCondition(item.dependencyResult)"
          v-for="(item, index) in componentResultList"
          :key="index"
        >
          ...
          <a-form-item :wrapperCol="{ span: 24 }" v-else-if="item.belong === 'view'" :colon="false" :label="null">
            <component :is="item.tag" :cwStyle="item.style" v-bind="[item.selfProps]"></component>
          </a-form-item>
          <a-form-item
            :wrapperCol="{ span: 24 }"
...
export default {
  ...
  watch: {
    ...
    value: {
      handler(val) {
        if ((this.mode !== 'dev' || !this.inIDE) && val) {
