...
namespace extensions.cw_dynamic_form_library.viewComponents {
  ...
  @Component({
    title: '带输入框的多选框',
    description: '带输入框的多选框',
  })
  export class CwAntdCheckboxWithInput<T, V> extends ViewComponent {
    constructor(options?: Partial<CwAntdCheckboxWithInputOptions<T, V>>) {
      super();
    }
  }

  export class CwAntdCheckboxWithInputOptions<T, V> extends ViewComponentOptions {
        @Prop({
      group: '数据属性',
      title: '数据类型',
