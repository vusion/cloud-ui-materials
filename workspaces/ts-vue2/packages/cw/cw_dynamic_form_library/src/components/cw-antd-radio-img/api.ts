...
namespace extensions.cw_dynamic_form_library.viewComponents {
  ...
  export class CwAntdRadioImg<T, V> extends ViewComponent {
    constructor(options?: Partial<CwAntdRadioImgOptions<T, V>>) {
      super();
    }
  }

  export class CwAntdRadioImgOptions<T, V> extends ViewComponentOptions {
        @Prop({
      group: '数据属性',
      title: '数据类型',
