...
import { repeatClick, clickOutside } from "@lcap-ui/src/directives";
import { noopFormatter, NumberFormatter } from "@lcap-ui/src/utils/Formatters";
...
export default {
  ...
  data() {
    ...
    if (this.formatter instanceof Object) data.currentFormatter = this.formatter;else if (typeof this.formatter === 'string') data.currentFormatter = new NumberFormatter(this.formatter);else data.currentFormatter = noopFormatter; // 初始值需要在最小值和最大值范围之内
  ...
  watch: {
    value(value, oldValue) {
      ...
      this.$emit('change', {
        ...
        valid: this.isValid(this.currentValue)
      }, this);
    },
    max(_value, oldValue) {
  ...
  methods: {
    ...
    toFixed(value) {
      // 为空时使用默认值
      if (typeof value === 'string' && value.trim() === '' || value === null || value === undefined) return value = this.defaultValue !== undefined ? this.defaultValue : null;else if (isNaN(value)) value = this.currentValue || this.defaultValue || 0;
    ...
    computePrecision(value) {
      // 优先使用精度设置的值
      if (this.precision !== 0 && this.step === 0) return this.precision;
      // 没有精度的情况下，需要判断value和step的值
      if (typeof value === 'string' && value.trim() === '' || value === null || value === undefined) value = this.defaultValue !== undefined ? this.defaultValue : this.currentValue || 0;else if (isNaN(value)) value = this.currentValue || this.defaultValue || 0;
