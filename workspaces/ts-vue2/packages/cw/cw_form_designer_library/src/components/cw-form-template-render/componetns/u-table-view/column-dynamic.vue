...
export default {
  ...
  destroyed() {
    this.$contact('u-table-view', parentVM => {
      ...
      this.$dispatch($parent => $parent.$options.name && $parent.$options.name === 'u-table-view', 'handle-columns');
    });
  },
  methods: {
    addVms() {
    ...
    reload() {
      ...
      if (this.currentDataSource.load) this.load();else if (this.currentDataSource.data)
        // valueField里的数据变化时，field需要重新生成
        this.addVms();
    }
  }
};
