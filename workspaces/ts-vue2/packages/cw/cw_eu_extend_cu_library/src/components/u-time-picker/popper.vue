...
export default {
  ...
  computed: {
    ...
    currentDisabled() {
      if (this.disabled) return true;else return false;
    }
  },
  watch: {
    time(newValue) {
      ...
      this.$emit('update', this.showTime);
    },
    showTime(newValue) {
      // if (!newValue)
    ...
    validShowTime(newValue) {
      this.$emit('update:inputTime', newValue);
    }
  },
  destroyed() {
  ...
  created() {
    this.$emit('update', this.getUnitFormatTime(this.time));
  },
  mounted() {
    this.setPopperWidth();
  },
  methods: {
    ...
    isOutOfRange(time) {
      ...
      return minTime && t < minTime && minTime || maxTime && t > maxTime && maxTime;
    },
    ...
    checkTime(time, strict = true) {
      ...
      if (strict) {
        if (this.minUnit === 'second') return secondReg.test(time);else return minuteReg.test(time);
      } else {
