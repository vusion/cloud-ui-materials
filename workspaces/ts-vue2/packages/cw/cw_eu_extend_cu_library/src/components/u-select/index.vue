...
import { UListView } from "@lcap-ui/src/components/u-list-view.vue";
import { ellipsisTitle } from "@lcap-ui/src/directives";
import i18n from "@lcap-ui/src/components/u-select.vue/i18n";
...
export default {
  ...
  extends: UListView,
  // i18n,
  mixins: [i18nMixin('u-select'), MPreview, sync({
    opened: 'popperOpened',
    preview: 'isPreview'
  })],
  props: {
  ...
  computed: {
    currentDisabled() {
      ...
      if (this.disabled || this.formVM && this.formVM.disabled) return true;else if (this.emptyDisabled) return this.currentData ? !this.currentData.length : !this.itemVMs.length;else return false;
    },
    filtering() {
      return {
    ...
    allChecked() {
      ...
      this.itemVMs.forEach(itemVM => {
        if (itemVM.currentSelected) checkedLength++;else if (itemVM.disabled || itemVM.readonly) disabledUnCheckedItemCount++;
      });
      if (!this.hasAllCheckItem) {
        disabledUnCheckedItemCount = 0;
      }
      if (checkedLength === 0) return false;else if (checkedLength + disabledUnCheckedItemCount === this.itemVMs.length) return true;else return null;
    }
  },
  watch: {
    ...
    value(value) {
      ...
      if (this.filterable && !this.multiple && this.currentDataSource && Array.isArray(this.currentDataSource.data)) {
        let selectedItem = this.currentDataSource.data.find(d => this.$at2(d, this.valueField) === value);
        let filterText = selectedItem ? this.$at2(selectedItem, this.field || this.textField) : '';
        if (!selectedItem && this.selectedValuesData && Array.isArray(this.selectedValuesData)) {
          selectedItem = this.selectedValuesData.find(itemData => itemData.value === value);
          if (selectedItem) {
  ...
  methods: {
    ...
    normalizeDataSource(dataSource) {
      ...
      if (dataSource instanceof DataSource || dataSource instanceof DataSourceNew) return dataSource;else if (dataSource instanceof Array) {
        ...
        options.load = function load(params, extraParams) {
          ...
          if (result instanceof Promise) return result.catch(() => this.currentLoading = false);else if (result instanceof Array) return Promise.resolve(result);else return Promise.resolve(result);
        };
        ...
        if (dataSource.hasOwnProperty('list') && Array.isArray(dataSource.list)) return new Constructor(Object.assign(options, dataSource, {
    ...
    selectByText(text) {
      ...
      if (this.multiple) {
        ...
        const selectedVM = this.itemVMs.find(itemVM => itemVM.currentText === text); // 如果没有匹配项则恢复到上一个状态
        if (!selectedVM && text) {
    ...
    prependItem(text) {
      this.currentDataSource.prepend({
        text,
        value: text
      });
    },
    ...
    clear() {
      ...
      if (this.multiple) {
        ...
        if (this.$emitPrevent('before-clear', {
          oldValue,
          value
        }, this)) return;
        ...
        this.$emit('clear', {
          oldValue,
          value: this.handleEmptyValue(value)
        }, this);
      }
      this.focus();
      this.close();
    ...
    onScroll(e) {
      ...
      const el = e.target;
      if (Math.abs(el.scrollHeight - (el.scrollTop + el.clientHeight)) <= 1 && this.currentDataSource && this.currentDataSource.hasMore()) this.debouncedLoad(true);
    },
    /**
    ...
    async loadMoreForSelected() {
      ...
      if (value && data) {
        ...
        if (Array.isArray(value)) {
          allInData = value.every(val => !!data.find(item => '' + this.$at2(item, this.valueField) === '' + val));
        } else {
          allInData = !!data.find(item => '' + this.$at2(item, this.valueField) === '' + value);
        }
