...
import IIco from "@lcap-ui/src/components/i-ico.vue";
...
export default {
  ...
  methods: {
    ...
    async onClick(e) {
      if (this.currentDisabled) return e.preventDefault();
    ...
    navigate(to) {
      ...
      if (!this.$router) return console.warn('[cloud-ui]', 'Cannot find vue-router.');
      let cancel = false;
      ...
      const {
        location
      } = $router.resolve(currentTo, this.$route, this.append);
