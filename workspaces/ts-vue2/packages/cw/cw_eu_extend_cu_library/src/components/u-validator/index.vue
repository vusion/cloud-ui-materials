...
export default {
  ...
  computed: {
    ...
    touched() {
      if (this.currentTarget === 'validatorVMs') return this.validatorVMs.some(validatorVM => validatorVM.touched);else return this.fieldTouched;
    },
    dirty() {
      if (this.currentTarget === 'validatorVMs') return this.validatorVMs.some(validatorVM => validatorVM.dirty);else return this.value !== this.oldValue;
    },
    valid() {
      if (this.currentTarget === 'validatorVMs') return this.validatorVMs.every(validatorVM => validatorVM.valid);else return this.realValid;
    },
    firstError() {
  ...
  created() {
    const context = this.$vnode.context;
    this.validator = new VusionValidator(context.$options.validators, context.$options.rules, this.currentRules, context); // this.debouncedValidate = debounce(this.validate, 50, { leading: false, trailing: true });
    ...
    if (this.$options.name !== 'u-form') this.$dispatch($parent => $parent.$options.isValidator || $parent.$options.isField, 'add-validator-vm', this);
    if (!this.parentVM || this.$options.name === 'u-form') this.rootVM = this;
  ...
  methods: {
    ...
    onChange($event) {
      ...
      if (!this.hasUpdateEvent && !this.inputing) this.validate('submit', true);
    },
    onFocus() {
    ...
    validate(trigger = 'submit', untouched = false) {
      if (this.currentTarget === 'validatorVMs') {
        return Promise.all(this.validatorVMs.map(validatorVM => validatorVM.validate('submit', untouched))).then(results => this.get$event(trigger));
      } else {
        ...
        if (untouched || this.muted === 'all' || this.muted === 'color') this.color = this.currentMessage = '';else this.color = this.state;
        ...
        return this.validator.validate(value, trigger, Object.assign({
          ...
          if (!untouched && this.muted !== 'all' && this.muted !== 'color') this.color = this.state;
          if (!untouched && this.muted !== 'all' && this.muted !== 'message') this.currentMessage = this.message;
          ...
          if (!untouched && this.muted !== 'all' && this.muted !== 'color') this.color = this.state;
          if (!untouched && this.muted !== 'all' && this.muted !== 'message') this.currentMessage = error;
          return this.onValidate(trigger);
        });
      }
    },
    ...
    onValidate(trigger) {
      ...
      if ($event.touched) {
        if ($event.trigger === 'blur') {
          if ($event.dirty) {
            if ($event.valid) this.$emit('blur-dirty-valid', $event, this);else this.$emit('blur-dirty-invalid', $event, this);
          }
          if ($event.valid) this.$emit('blur-valid', $event, this);else this.$emit('blur-invalid', $event, this);
        } else if ($event.trigger === 'input') {
          if ($event.dirty) {
            if ($event.valid) this.$emit('input-dirty-valid', $event, this);else this.$emit('input-dirty-invalid', $event, this);
          }
          if ($event.valid) this.$emit('input-valid', $event, this);else this.$emit('input-invalid', $event, this);
        } else if ($event.trigger === 'submit') {
          if ($event.dirty) {
            if ($event.valid) this.$emit('submit-dirty-valid', $event, this);else this.$emit('submit-dirty-invalid', $event, this);
          }
          if ($event.valid) this.$emit('submit-valid', $event, this);else this.$emit('submit-invalid', $event, this);
        }
      }
