<template>
    <div :class="$style.root">
        <div v-if="draggable && !isPreview && (!readonly || $env.VUE_APP_DESIGNER)" :class="$style.draggable"
            :dragover="dragover" @click="select()" :tabindex="readonly || disabled ? '' : 0" @drop.prevent="onDrop"
            @paste="onPaste" @dragover.prevent="dragover = true" @dragleave.prevent="dragover = false">
            <input :class="$style.file" ref="file" type="file" :name="name" :accept="accept" :multiple="multiple"
                :readonly="readonly" :disabled="disabled" @click.stop @change="onChange">
            <div>
                <div v-if="dragDescription" vusion-slot-name="dragDescription" :class="$style.dragDescription">
                    <slot name="dragDescription">{{ dragDescription }}</slot>
                </div>
                <slot></slot>
            </div>
        </div>
        <div v-else-if="listType !== 'card' && !isPreview && (!readonly || $env.VUE_APP_DESIGNER)"
            :class="$style.select" @click="select()" vusion-slot-name="default"
                    ...
                    <template v-if="item.status !== 'error'">
                        <div :class="$style.textContainer" v-if="listType === 'text' && $slots['file-list']">
                            <span v-for="flag in fileListFlags" :key="flag" :class="$style[flag]">
                                <component v-if="flag !== 'download-icon' && isShowFileListItem(flag)"
                                    :style="fileListStyleInfos[flag]" :is="fileListComponentFlagMap[flag].is"
                                    v-bind="fileListComponentFlagMap[flag].getProps(item)"></component>
                                <a v-else-if="downloadIconSwitcher && isShowFileListItem(flag)"
                                    :style="fileListStyleInfos['download-icon']" :href="encodeUrl(item.url)"
                                    target="_blank" download role="download">
                                    <i-ico :name="downloadIcon" icotype="only"></i-ico>
                                </a>
                            </span>
                            <i-ico name="remove" :class="$style.remove"
                                v-if="!readonly && !disabled && !isPreview && !$env.VUE_APP_DESIGNER"
                                @click="remove(index)"></i-ico>
                        </div>
                        <div v-else>
                            <div :class="$style.thumb"><img :class="$style.img" v-if="listType === 'image'"
                                    :src="getUrl(item)"></div>
                            <a :class="$style.link" :href="encodeUrl(item.url)" target="_blank" download
                                role="download">{{ item.name || item.url }}</a>
                            <i-ico name="remove" v-if="!readonly && !disabled && !isPreview" :class="$style.remove"
                                @click="remove(index)"></i-ico>
                        </div>
                        <u-linear-progress v-if="item.showProgress && !$env.VUE_APP_DESIGNER" :class="$style.progress"
                            :percent="item.percent"></u-linear-progress>
                    </template>
                </div>
            </template>
            <template v-else>
                <div :class="$style.card" v-for="(item, index) in currentValue" :key="index"
                    @click="!multiple && !readonly && select()">
                    ...
                    <div :class="$style.mask" :multiple="multiple || readonly" :show-progress="item.showProgress">
                        <u-linear-progress v-if="item.showProgress" :class="$style.progress"
                            :percent="item.percent"></u-linear-progress>
                        <div :class="$style.buttons">
                            <span v-if="!readonly && !disabled && !isPreview" :class="$style.button" role="remove"
                                @click.stop="remove(index)"></span>
                            <span :class="$style.button" role="preview" @click.stop="onPreview(item, index)"></span>
                            <a v-if="downLoadFilename" :class="$style.button" @click.stop :href="encodeUrl(item.url)"
                                target="_blank" role="download" :download="downLoadFilename"></a>
                            <a v-else :class="$style.button" :href="encodeUrl(item.url)" @click.stop target="_blank"
                                download role="download"></a>
                        </div>
                    </div>
                </div>
                <div :class="$style.cardwrap" v-if="uploadEnable && !draggable && (!readonly || $env.VUE_APP_DESIGNER)">
                    <div :class="$style.card" role="select" @click="select()">
                        <input :class="$style.file" ref="file" type="file" :name="name" :accept="accept"
                            :multiple="multiple" :readonly="readonly" :disabled="disabled" @click.stop
                            @change="onChange">
                    </div>
                    <div v-if="description" :class="$style.description">{{ description }}</div>
                    <f-scroll-view trigger="hover" v-if="showErrorMessage && errorMessage.length">
                        <div :class="$style.errwrap">
                            <div v-for="errItem in errorMessage" :key="errItem" :class="$style.errmessage">{{ errItem }}
                            </div>
                        </div>
                    </f-scroll-view>
                </div>
            </template>
            <span v-show="(showInDesigner || $env.VUE_APP_DESIGNER) && listType === 'text' && !isPreview"
                vusion-slot-name="file-list">
                <slot name="file-list" ref="file-list"></slot>
            </span>
        </div>
        <u-lightbox :visible.sync="lightboxVisible" :value="currentIndex" animation="fade">
            <u-lightbox-item v-for="(item, index) in currentValue" :key="index" :value="index" :title="item.name"><img
                    :src="encodeUrl(item.url || item)"></u-lightbox-item>
        </u-lightbox>
        <cropper v-if="openCropper" :crop-file-name="cropFileName" :crop-img="cropImg" :crop-config="cropConfig"
            :modal-visible="modalVisible" @uploadFiles="uploadCropperImg">
        </cropper>
...
export default {
    ...
    props: {
        ...
        fileConnectionGroup: { type: String, default: '' },
    },
    data() {
        return {
