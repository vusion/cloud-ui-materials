<template>
  <th v-if="columnVM && columnVM.colSpan !== 0" ref="th"
    :class="[$style['head-title'], boldHeader ? $style.boldHeader : null]"
    :data-excel-col="excelHeadColIndex >= 0 ? excelHeadColIndex : undefined" :key="columnIndex"
    :sortable="columnVM.sortable && sortTrigger === 'head'" :filterable="!!columnVM.filters"
    @click="onHeadClick(columnVM, $event)" :disabled="columnVM.currentHidden" :colspan="columnVM.colSpan"
    :rowspan="columnVM.rowSpan" :ellipsis="columnVM.thEllipsis !== undefined ? columnVM.thEllipsis : thEllipsis"
    v-ellipsis-title :thtextalign="columnVM.thtextalign">
    <!-- type === 'checkbox' -->
    <span v-if="columnVM.type === 'checkbox'">
      <u-checkbox :value="allChecked" @check="checkAll($event.value)" :disabled="disabled"
        :readonly="readonly"></u-checkbox>
    </span>
    <!-- Normal title -->
    <template>
    ...
    <span v-if="columnVM.sortable" :class="$style.sort"
      :sorting="currentSorting && currentSorting.field === columnVM.field"
      :order="currentSorting && currentSorting.order"
    ...
    <span v-if="columnVM.filters" :class="$style['filter-wrap']" :active="isFilterActive(columnVM.field)">
      <u-table-view-filters-popper :value="getFiltersValue(columnVM.field)" :data="columnVM.filters"
        :multiple="columnVM.filterMultiple || filterMultiple" :max="columnVM.filterMax || filterMax"
    ...
    <f-dragger v-if="resizable && columnIndex !== headTrLength - 1" axis="horizontal"
      @dragstart="onResizerDragStart($event, columnVM)" @drag="onResizerDrag($event, columnVM, columnIndex)"
