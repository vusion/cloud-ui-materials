...
// 单函数插件：文件名即类型
rules.push([is('w:p'), parseParagraph, 'w:p']);
rules.push([is('w:hyperlink'), hyperlink, 'w:hyperlink']);

// 多类型对象插件（表格相关）
Object.entries(tbl).forEach(([type, fn]) => {
    rules.push([is(type), fn, type]);
});
