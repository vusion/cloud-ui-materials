<template>
  <div :class="$style.root" :animation="animation" :readonly="readonly" :disabled="disabled">
    <u-multi-layout vusion-node-tag="u-multi-layout">
      <slot name="left"></slot>
        ...
        <template v-if="childrenNodes.length > 0">
          <template v-for="(node, idx) in childrenNodes">
            <template v-if="optionIsSlot">
              ...
              >
                <template #navbarNode="slotProps">
                  <slot name="navbarNode" v-bind="slotProps"></slot>
                </template>
...
import { MGroupParent } from '@lcap-ui/src/components/m-group.vue';
import { MSinglex } from '@lcap-ui/src/components/m-singlex.vue';
import UNavbarDynamicGroup from './dynamic-group.vue';
...
export default {
  ...
  computed: {
    ...
    mockDataSource() {
      if (this.$env.VUE_APP_DESIGNER && this.optionIsSlot) {
        return {
          data: [
            {
              children: [{ children: [{}] }],
  ...
  created() {
    ...
    this.currentDataSource = this.mockDataSource || this.normalizeDataSource(this.dataSource || this.data);
  ...
  methods: {
    watchValue(value) {
      if (this.selectedVM && this.selectedVM.value === value)
        ...
        return;
      if (value === undefined) {
        if (this.autoSelect) this.selectedVM = this.itemVMs.find((itemVM) => !itemVM.disabled) || undefined;
        ...
        if (!this.selectedVM && this.selectedValuesData && Array.isArray(this.selectedValuesData)) {
          this.selectedVM = this.selectedValuesData.find((itemData) => itemData.value === value);
        }
