...
import i18n from "@lcap-ui/src/components/u-uploader.vue/i18n";
import ajax from './ajax';
import cropper from "@lcap-ui/src/components/u-uploader.vue/cropper";
import i18nMixin from "@lcap-ui/src/mixins/i18n";
import FileImg from "@lcap-ui/src/components/u-uploader.vue/assets/file.svg";
...
export default {
  ...
  mixins: [MField, i18nMixin('u-uploader'), MPreview, sync({
    value() {
  ...
  methods: {
    ...
    fromValue(value) {
      ...
      if (this.converter === 'json') try {
        const parsedValue = JSON.parse(value || '[]');
        return Array.isArray(parsedValue) ? parsedValue : [];
      } catch (err) {
        return [];
      } else if (this.converter === 'simple') try {
        if (!value) {
    ...
    toValue(value) {
      ...
      if (this.converter === 'simple') return Array.isArray(value) && value.length === 0 ? null : this.simpleConvert(value);else return value;
    },
    simpleConvert(value) {
    ...
    checkSize(file) {
      ...
      if (!isNaN(this.maxSize)) maxSize = +this.maxSize;else {
        ...
        if (!SIZE_UNITS[unit]) throw new Error(`Unknown unit ${unit} in maxSize ${this.maxSize}!`);
    ...
    async uploadFiles(files) {
      ...
      if (count > this.limit) {
        this.errorMessage[0] = this.$tt('limitError', {
          limit: this.limit,
          count
        });
    ...
    uploadCropperImg(obj) {
      ...
      //     progress: 100,
      // });
      const file = new window.File([obj.blob], obj.name, {
        type: 'image/jpeg'
      });
    ...
    upload(file) {
      ...
      if (this.listType === 'image' || this.listType === 'card') item.url = URL.createObjectURL(file);
      if (!this.multiple) this.currentValue.splice(0, this.currentValue.length);
      this.currentValue.push(item);
      if (this.autoUpload) this.post(file, item, this.currentValue.length - 1);
      return item;
    },
    /**
     * multipleOnce 的场景
    ...
    uploadOnce(files) {
      ...
      if (!this.multiple) this.currentValue.splice(0, this.currentValue.length);
      this.currentValue.push(item);
      if (this.autoUpload) this.post(files, item, this.currentValue.length - 1);
      return item;
    },
    post(file, item, index) {
      ...
      if (window.appInfo && window.appInfo.domainName) headers.DomainName = window.appInfo.domainName;
      ...
      const requestData = {
        ...
        file: Array.isArray(file) ? file.map(item => new File([item], item.name.replace(/[#\+]/g, ''), {
          type: item.type
      ...
      const xhr = ajax({
        ...
        onError: (e, res) => {
          ...
          item.status = 'error';
          this.emitInputEvent();
          const defaultErrMsg = this.$tt('requestError', {
            fileName: item.name
          });
    ...
    onPaste(e) {
      if (this.readonly || this.disabled) return;
      if (this.pastable) this.uploadFiles(e.clipboardData.files);
    },
    /**
    ...
    async checkFiles(files) {
      ...
      const tasks = files.map(async file => {
        if (!this.checkSize(file)) {
          const errorMessage = this.$tt('maxSizeError', {
            fileName: file.name,
        ...
        if (this.accept) {
          ...
          const accept = this.accept.split(',').map(type => type.trim()).filter(type => type).some(acceptedType => {
          ...
          if (!accept) {
            this.errorMessage.push(this.$tt('acceptError', {
              accept: this.accept
            }));
            return null;
          }
        ...
        // 触发自定义校验文件事件
        if (this.$listeners && this.$listeners['custom-check-file']) {
          const result = await new Promise((resolve) => {
            let callbackCalled = false;
            
            // 触发事件，传递文件数据和回调函数
            this.$emit('custom-check-file', {
              file,
              message: null,
              name: file.name,
              callback: (errorMessage) => {
                // 父组件异步校验完成后调用回调，传递错误消息（字符串）或 null（通过）
                if (!callbackCalled) {
                  callbackCalled = true;
