...
namespace extensions.cust_dahua_uploader_limit_library.viewComponents {
  const {
    Component,
    Prop,
    ViewComponent,
  ...
  @ExtensionComponent({
    replaceNaslUIComponent: "UUploader",
    type: "pc",
    show: true,
    ideusage: {
      "idetype": "container",
  ...
  export class ExUUploaderOptions extends ViewComponentOptions {
    ...
    @Event({
      title: '自定义文件校验',
      description: '自定义文件校验时触发，可通过返回错误消息来阻止上传'
    })
    onCustomCheckFile: (event: {
      file: nasl.ui.File;
      message: nasl.core.String;
      name: nasl.core.String;
      callback: any;
