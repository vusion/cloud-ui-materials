...
export default {
  ...
  methods: {
    initiialize() {
      ...
      this.renderJFlow();
      this.syncScrollBarsWithCanvas();
      this.syncZoom();
    },
    /** 与 resize 一致：滚动容器尺寸/范围与当前画布包围盒对齐，避免 Fit/平移后跳动 */
    syncScrollBarsWithCanvas() {
      const jflow = this.$refs.jflow?.getInstance();
      if (jflow && typeof jflow.scrollBarOnPanAndZoom === 'function') {
    ...
    onFit() {
      ...
      // 优先使用 layout 预算的包围盒：有泳道标题时为外框 bounds（与 FrameGroup.render 一致）；
      // 无标题时为纯业务节点 contentBBox（无外框留白）。均可能比 jflow.bounding_box 更可靠。
      const layoutBounds = this.configs.layout._frameBounds;
      const {
        width: p_width,
        height: p_height,
        x: p_x,
        y: p_y
      } = layoutBounds || jflowInstance.bounding_box;
      const MIN = 1e-6;
      if (
        !Number.isFinite(p_width) ||
        !Number.isFinite(p_height) ||
        !Number.isFinite(p_x) ||
        !Number.isFinite(p_y) ||
        p_width <= MIN ||
        p_height <= MIN
