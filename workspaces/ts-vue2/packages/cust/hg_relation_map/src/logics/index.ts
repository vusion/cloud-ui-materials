...
/**
 * 将数字转换为英文大写（纯小数去掉Zero/and，有小数加Cents，无小数无单位，末尾加ONLY）
 * 支持：整数、小数、负数，范围：-999,999,999,999.99 到 999,999,999,999.99
...
function numberToEnglishWords(num) {
    ...
    // 拆分整数和小数部分（保留两位小数，避免浮点精度问题）
    const integerPart = Math.floor(num);
    const decimalPart = Math.round((num - integerPart) * 100); // 四舍五入到分
    ...
    if (decimalPart > 0) {
        if (decimalPart < 20) {
            englishDecimal = ones[decimalPart];
        } else {
            const tenDigit = Math.floor(decimalPart / 10);
            const oneDigit = decimalPart % 10;
            englishDecimal = tens[tenDigit] + (oneDigit !== 0 ? " " + ones[oneDigit] : "");
        }
        englishDecimal += " Cents"; // 有小数时保留Cents单位
    }

    // 拼接最终结果（核心修改：纯小数去掉Zero/and）
    let finalResult = "";
    if (isNegative) {
        finalResult += "Negative ";
    }

    // 场景1：纯小数（整数部分为0，小数部分>0）
    if (integerPart === 0 && decimalPart > 0) {
        finalResult += englishDecimal;
    } 
    // 场景2：纯整数（小数部分为0）
    else if (decimalPart === 0) {
        finalResult += englishInteger;
    } 
    // 场景3：带整数的小数（整数>0，小数>0）
    else {
        finalResult += englishInteger + " and " + englishDecimal;
    }
