...
import encodeUrl from "@lcap-ui/src/utils/encodeUrl";
import { onlineSvgIcon, ie11 } from "@lcap-ui/src/components/i-ico.vue/online-svg-icon-vue2/src/main.js";
export default {
  name: 'i-ico',
  components: {
    SEmpty,
  ...
  methods: {
    ...
    async onClick(ev) {
      ...
      function currentHref() {
        if (props.href !== undefined) return encodeUrl(props.href);else if (parent && parent.$router && props.to !== undefined) return encodeUrl(parent && parent.$router.resolve(props.to, parent && parent.$route, props.append).href);else return undefined;
      }
      ...
      if (hrefR === undefined) {
        ...
        const {
          location,
          href
        } = $router.resolve(currentTo, $route, props.append);
