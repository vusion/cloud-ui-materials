...
export default {
  ...
  methods: {
    ...
    generateSeriesData(data, multiYAxisList) {
      ...
      const yAxisData = this.getAxisData(data, this.axisData.yAxis);
      xAxisData.forEach((item, index) => {
        seriesData.push([xAxisData[index], yAxisData[index]])
      })
    ...
    processLineData(data) {
      ...
      this.scatterOption = this.generateEchartOption(legendData, seriesData, xAxisData);
    },
    ...
    toolTipFormatter(params) {
      ...
      let template= '';
      if (this.$env.VUE_APP_DESIGNER || !window.appInfo) {
        legendAliasList = (legendAliasList.length !== 0 && multiYAxisList.length === legendAliasList.length) ? ['别名1', '别名2'] : ['指标1', '指标2'];
      ...
      for (let index=0; index<params.length; index++) {
        ...
        template += `<div style="color: ${params[index].color}">
                         <div>${legendAliasList[0]}: <b style="float: right; margin-left: 20px;"> ${params[index].value[0]}</b></div>
                         <div>${legendAliasList[1]}: <b style="float: right; margin-left: 20px;"> ${params[index].value[1]}</b></div>
                     </div>`
      }
      return template;
    },
