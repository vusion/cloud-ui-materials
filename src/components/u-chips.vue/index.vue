...
export default {
    ...
    props: {
        ...
        allowEmpty: { type: Boolean, default: true }, // 老的验证体系下，关于是否允许为空的属性
        error: String, // 老的验证体系下，空值时的错误提示
        value: { type: Array, default: () => [] }, // chips数组内容
    ...
    computed: {
        ...
        maxHeight() {
            if (this.type === 'searchInput') {
                ...
                if (this.isFocused) return 150; // 滚动到最顶上
    ...
    created() {
        ...
        if (this.type === 'searchInput')
            window.addEventListener('mousedown', this.onDocMousedown, false); // 是否需要保持错误信息
    ...
    methods: {
         ...
         */ onDocKeydown(event) {
            ...
            if (event.which === 39) {
                ...
                if (current === list.length - 1) {
                    current = -1;
                    this.$refs.cpInput.focus(); // 向右切换生成项的聚焦
                } else this.onFocus(current + 1);
            } // 键盘左键
            if (event.which === 37) {
                // 左边界，不再往左移动生成项聚焦
                if (current === 0) return; // 向左切换生成项的聚焦
         ...
         */ onModifyKeydown(event) {
            ...
            this.errMessage = ''; // enter键
            // 禁止默认enter键的默认操作
            if (event.which === 13) event.preventDefault(); // 空格键  生成项
            if (
                (event.which === 32 && !this.noSpace) ||
                (event.which === 188 && !this.noComma) ||
                (this.type === 'searchInput' && event.which === 13)
         ...
         */ onFocus(index, $event) {
            ...
            this.current = index; // 这里是因为注册在document上的keydown事件，需要手动 $update
        },
        /**
         * 双击生成项，变为编辑状态
         * @param {number} index - 生成项的索引
         ...
         */ onDBLClick(index, event) {
            ...
            this.current = index; // 在list当中去除当前的编辑项
         ...
         */ generate(item, isModify = false, validator, trigger = 'blur') {
            if (this.type === 'searchInput' && /^\s*$/.test(item)) return; // item == false，说明item为空字符串或空格组成的字符串
            if (item === '' && item !== '0') {
                ...
                itemArr.every((itm, index) => {
                    ...
                    if (this.errMessage) {
                        ...
                        if (isModify) {
                            ...
                            this.list.splice(this.current, 0, itm); // 创建新生成项
         ...
         */ validate(value, type = 'input', list) {
            list = list || this.list; // 空值或已经有错误信息不检测
