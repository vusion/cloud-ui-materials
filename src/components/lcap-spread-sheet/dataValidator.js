...
const { DefaultDataValidator } = GC.Spread.Sheets.DataValidation;
import { isSelectWidget } from './CustomCell';
 ...
 */
export function widgetValidator(widgetSetting, value, type, options = { ignoreBlank: true }) {
    let valid = true;
    let message = '';
    if (!widgetSetting) {
...
export function defProperty(propertyName, initValue) {
    const pname = propertyName;
    const defValue = initValue;
    const t = function (value, shouldCallback) {
        const self = this;
        if (!self.hasOwnProperty('_ps')) {
            self._ps = {};
        }
        const properties = self._ps;
        if (arguments.length === 0) {
            if (properties[pname] !== undefined) {
...
DefaultDataValidator.prototype.initWidgetValidator = function (widgetType, setting) {
    this.widgetType = defProperty('widgetType');
    this.widgetSetting = defProperty('widgetSetting');
    this.widgetType(widgetType);
    this.widgetSetting(setting);
    const bindValidator = this;
    const condition = this.condition();
    condition.evaluate = function (evaluator, baseRow, baseColumn, actualObj) {
        return widgetValidatorEvaulate(evaluator, baseRow, baseColumn, actualObj, bindValidator);
    };
};

const prevFromJSON = DefaultDataValidator.prototype.fromJSON;
const prevToJSON = DefaultDataValidator.prototype.toJSON;
...
DefaultDataValidator.prototype.fromJSON = function (settings, noSchema, context) {
    prevFromJSON.call(this, settings, noSchema, context);
    if (this.type() === 20) {
        const { widgetType, widgetSetting } = settings;
        if (widgetType === 'Date') {
            transformDateSetting(widgetSetting);
        }
        this.initWidgetValidator(settings.widgetType, settings.widgetSetting);
    }
};

DefaultDataValidator.prototype.toJSON = function (context) {
    const jsonData = prevToJSON.call(this, context);
    if (this.type() === 20) {
        jsonData.widgetType = this.widgetType();
        jsonData.widgetSetting = this.widgetSetting();
    }
    return jsonData;
};
