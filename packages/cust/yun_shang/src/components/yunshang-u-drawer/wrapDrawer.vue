<template>
  ...
  <component v-else is="u-drawer-real" v-bind="[$attrs, $props]" v-on="$listeners" ref="item" open-event="openEvent">
    <slot v-for="(item, name) in $slots" :name="name" :slot="name"></slot>
    <template v-for="(_, name) in $scopedSlots" :slot="name" slot-scope="slotData">
      <slot :name="name" v-bind="slotData"></slot>
    </template>
...
import i18n from '@lcap-ui/src/components/u-drawer.vue/i18n';
...
export const UDrawer = {
  ...
  methods: {
    ...
    getDynamicPlaceholderInDesigner() {
      ...
      this.placeholderInDesigner = [title && `标题：${title}`, refName && `组件名称：${refName}`, '双击编辑抽屉'].filter(Boolean).join('，'); // 注意中文的符号
