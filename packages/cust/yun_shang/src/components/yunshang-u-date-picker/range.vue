...
export default {
  ...
  mixins: [MField, MPreview, DateFormatMixin, i18nMixin('u-date-picker'), sync({
    startDate() {
  ...
  watch: {
    ...
    showStartDate(newValue) {
      ...
      this.$emit('input', newDate && this.showEndDate ? [this.toValue(newDate), this.toValue(new Date(this.transformDate(this.showEndDate)))] : '');
    ...
    showEndDate(newValue) {
      ...
      this.$emit('input', this.showStartDate && newDate ? [this.toValue(new Date(this.transformDate(this.showStartDate))), this.toValue(newDate)] : '');
  ...
  created() {
    ...
    this.$emit('update', this.showStartDate && this.showEndDate ? [this.toValue(this.showStartDate ? new Date(this.transformDate(this.showStartDate)) : ''), this.toValue(this.showEndDate ? new Date(this.transformDate(this.showEndDate)) : '')] : '');
  },
  mounted() {
  ...
  methods: {
    ...
    select({
      ...
      if (this.readonly || this.disabled || this.isOutOfRange(startDate) || this.isOutOfRange(endDate)) return;
    ...
    onInput({
      ...
      // 处理 left > right 的情况
      if (this.checkValid(leftValue) && this.checkValid(rightValue) && new Date(this.transformDate(leftValue)) > new Date(this.transformDate(rightValue))) {
      ...
      if (this.checkValid(leftValue)) {
        ...
        this.$refs.input.updateCurrentValue({
          leftValue: this.genDisplayFormatText(this.showStartDate)
        });
      }
      if (this.checkValid(rightValue)) {
        ...
        this.$refs.input.updateCurrentValue({
          rightValue: this.genDisplayFormatText(this.showEndDate)
        });
      }
    },
    /**
