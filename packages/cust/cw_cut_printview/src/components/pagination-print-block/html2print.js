...
export default class Html2Pdf {
  ...
  async print() {
    ...
    for (let i = 0; i < newPages.length; ++i) {
      ...
      if (i < newPages.length - 1) {
        ...
        // 对多余的内容部分进行遮白
        this.addBlank(0, imageHeight + pdfHeaderH + baseY, this.pagerWidth, this.pagerHeight - imageHeight - baseY, pdf);
      }
  ...
  traversingNodes(nodes) {
    for (const element of nodes) {
      ...
      if (isSplit) {
        ...
        this.pages.push(splitBottomRate);
        // 继续处理后面的元素，不return
        continue;
      }
      // dom转换后距离顶部的高度
  ...
  async addFooter(pageSize, pageNo, footer, pdf, contentWidth) {
    ...
    pdf.addImage(data, 'JPEG', this.baseX, this.pagerHeight - height - this.baseY, contentWidth, height);
  }
