<script>
import FVirtualList from "@lcap-ui/src/components/f-virtual-list.vue";
export default {
  ...
  computed: {
    virtualList() {
      ...
      if (!this.virtual) return list;else {
        let count = 0;
  ...
  methods: {
    handleVirtualScroll(e) {
      ...
      if (this.virtualReseted) {
        this.virtualReseted = false;
        return;
      }
      ...
      if (this.itemHeight === undefined) {
        ...
        children.forEach((childEl, index) => {
          ...
          if (item && item.height === undefined && item._cacheHeight === undefined) item._cacheHeight = item.height || childEl.offsetHeight;
        });
      }
      const getHeight = item => {
        if (item.display === 'none') return 0;else if (this.itemHeight !== undefined) return this.itemHeight;else if (item.height !== undefined) return item.height;else if (item._cacheHeight !== undefined) return item._cacheHeight;else return 0;
      };
      ...
      virtualIndex = Math.max(0, currentIndex - Math.ceil((this.virtualCount - showCount) / 2)); // eslint-disable-next-line yoda
