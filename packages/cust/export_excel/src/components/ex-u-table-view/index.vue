...
export default {
  ...
  methods: {
    ...
    async exportExcel(page = 1, size = 2000, filename, sort, order, excludeColumns = [], includeStyles = false, watermarkText , watermarkImageBase64, textAlpha , imageAlpha) {
      ...
      // 限制透明度的范围在0-1之间
      textAlpha = textAlpha == null ? 1 : Math.min(1, Math.max(0, textAlpha));
      imageAlpha = imageAlpha == null ? 1 : Math.min(1, Math.max(0, imageAlpha));
      const fn = event => {
      ...
      try {
        ...
        // 如果有水印文本，使用ExcelJS处理
        if (watermarkText || watermarkImageBase64) {
          
          const workbook = new ExcelJS.Workbook();
          const worksheet = workbook.addWorksheet('Sheet1');
          
          // 添加水印
          const watermarkImage = await createWatermark(watermarkText, watermarkImageBase64, textAlpha, imageAlpha);
          if (watermarkImage) {
            const imageId = workbook.addImage({
              base64: watermarkImage,
              extension: 'png'
            });
          ...
          if (hasHeader && content.length > 0) {
            const headerRows = content.slice(0, headerRowCount);
            headerRows.forEach((row, rowIndex) => {
              const excelRow = worksheet.addRow(row.map(cell => cell.v || ''));
              excelRow.font = { bold: true };
            });
          }
          
          // 添加数据行
          const dataRows = content.slice(headerRowCount);
          dataRows.forEach(row => {
            worksheet.addRow(row.map(cell => cell.v || ''));
          });
          
          // 处理合并单元格
          if (mergesMap && mergesMap.length > 0) {
            mergesMap.forEach(merge => {
