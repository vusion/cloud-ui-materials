  ...
  <el-config-provider :locale="zhCn" namespace="huadian">
    <div :class="$style.setterRoot">
      ...
      <el-dialog
        ...
        modal-class="huadian-setter-dialog"
        title="选择图标"
        width="1200"
        :align-center="true"
        @open="handleOpen"
      ...
      >
        <div :class="$style.container">
          <el-tabs v-model="activeName" @tab-change="changeTab">
            <el-tab-pane label="插画库" name="library">
              <!-- <IconLibrary ref="iconLibraryRef" @showDetail="handleLibraryShowDetail" @select="handleLibrarySelect" /> -->
            </el-tab-pane>
            <el-tab-pane label="上传插画" name="upload">
              <UploadImage ref="uploadIconRef" :value="value" @select="handleUploadSelect" />
            </el-tab-pane>
          </el-tabs>
        </div>
        <template #footer>
...
import UploadImage from '@/components/ExImageSetter/UploadImage.vue';
...
const placeholder = computed(() => {
  return showClearIcon.value ? '' : '请选择插画';
});
