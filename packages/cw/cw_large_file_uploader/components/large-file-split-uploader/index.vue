...
export default {
    ...
    props: {
        value: [String, Object],
        url: { type: String, required: true, default: '/split/upload' },
        ...
        upload(file) {
            ...
            if (
                ...
                this.$children.some(
                    (item) =>
                        item &&
                        item.$attrs.flag === 'large-file-uploader-button'
                )
            ) {
        ...
        async uploadChunk(file) {
            ...
            for (
                ...
                if (chunkNumber === totalChunks - 1) {
                    if (
                        file.size - start < minLastChunkSize &&
                        totalChunks > 1
                    ) {
                ...
                const xhr = ajax({
                    ...
                    onProgress: (e) => {
                        this.currentValue = {
                            ...
                            status: 'uploading',
                            percent: parseInt(
                                percent + e.percent / (totalChunks + 1)
                            ),
                            showProgress: true,
                        };
                        this.$emit(
