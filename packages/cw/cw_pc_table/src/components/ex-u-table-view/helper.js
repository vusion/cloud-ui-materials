import Color from "@lcap-ui/src/components/u-table-view.vue/Color";
export const createTableHeaderExportHelper = rowCount => {
  const emptyPlaceholder = {};
  let headContents = Array.from({
    length: rowCount
  }, () => []);
  let rowIndex = 0;
  let colIndex = 0;
  ...
  function setCell(content, isCurrentRowEnd = false, rowSpan = 1, colSpan = 1) {
    ...
    while (headContents[searchRowStart] && headContents[searchRowStart][searchColStart] === emptyPlaceholder) searchColStart++;
  ...
  function eject() {
    ejectFlag = true;
    const matrix = headContents.map(arr => arr.map(v => v === emptyPlaceholder ? null : v));
    headContents = null;
    return matrix;
  }
};
