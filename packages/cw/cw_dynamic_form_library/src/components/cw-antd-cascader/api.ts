...
namespace extensions.cw_dynamic_form_library.viewComponents {
  ...
  export class CwAntdCascader<T, V> extends ViewComponent {
    constructor(options?: Partial<CwAntdCascaderOptions<T, V>>) {
      super();
    }
  }

  export class CwAntdCascaderOptions<T, V> extends ViewComponentOptions {
        @Prop({
      group: '数据属性',
      title: '数据类型',
