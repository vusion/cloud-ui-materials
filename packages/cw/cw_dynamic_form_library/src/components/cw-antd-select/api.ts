...
namespace extensions.cw_dynamic_form_library.viewComponents {
  ...
  export class CwAntdSelect<T, V> extends ViewComponent {
    constructor(options?: Partial<CwAntdSelectOptions<T, V>>) {
      super();
    }
  }

  export class CwAntdSelectOptions<T, V> extends ViewComponentOptions {
    @Prop({
      group: '数据属性',
      title: '数据源',
    ...
    @Prop<CwAntdSelectOptions<T, V>, 'valueField'>({
      group: '数据属性',
      title: '值字段',
    ...
    @Prop<CwAntdSelectOptions<T, V>, 'textField'>({
      group: '数据属性',
      title: '文本字段',
