<template>
  <div style="width:100%">
    <a-checkbox-group
      ...
      @change="handleChange"
    >
      <a-checkbox v-for="(item,index) in sourceList" :key="index" :value="item.value">
        {{ item.label }}
      </a-checkbox>
      <div class="othor-radio-room" v-if="otherData">
        <a-checkbox :value="otherData">
          <span style="white-space: nowrap;">{{ otherData }}</span>
        </a-checkbox>
        <a-input
          v-if="!linkage||(currentValue||[]).includes(otherData)"
