...
import { createBranchGroup } from './flow-branch-group.js';
...
class FlowLink extends BaseLink {
    constructor(configs) {
        ...
        this.branchInfo = configs.branchInfo;
    ...
    _cacheBranchInfo(){
        if(this.branchInfo) {
            if(!this._cacheBranchBlock) {
                this._cacheBranchBlock = createBranchGroup(this.branchInfo);
                const n = this._cacheBranchBlock.getNode();
                const q = this;
                Object.defineProperty(n, '_belongs', {
                    get() {
                        return q._jflow;
                    }
                });
            } else {
                this._cacheBranchBlock.setBranchInfo(this.branchInfo);
            }
        }
    }
    // _onRefreshCachePoint() {
    //     this._cacheSegments = segmentDistances(this._cachePoints);
    //     this._cacheMiddlePoint = getMiddlePointOfSegments(this._cacheSegments);
    //     this._cacheBoundingRect = linkBoundingRect(this._cachePoints);
    // }
    ...
    render(ctx) {
        ...
        if (this.showAdd) {
            ...
            const branchBlock = this._cacheBranchBlock.getNode();
            const width = branchBlock.width;
            const height = branchBlock.height
            const region = new Path2D();
        ...
        if (_showContent) {
            if(this._cacheBranchBlock) {
                const branchBlock = this._cacheBranchBlock.getNode();
                branchBlock.anchor = middlePoint;
                branchBlock.render(ctx);
            }
        }
    }
